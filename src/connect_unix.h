...
int server_add_stream_client( Connection* connection, const char* key )
{
  ...
  if( FD_ISSET( connection->sockfd, &read_fds ) )
    client_sockfd = accept( connection->sockfd, (struct sockaddr *)(client_info.ai_addr), &(client_info.ai_addrlen) );
  ...
  {    
    memset( connection->buffer, 0, BUFFER_SIZE );
    // Blocks until there is something to be read in the socket
    if( recv( client_sockfd, connection->buffer, BUFFER_SIZE, 0 ) == SOCKET_ERROR )
    {
...
int open_connection( const char* address, const char* port, int type )
{
  ...
  {
    perror("open_connection: couldn't read host info for socket start");
    fprintf( stderr, "open_connection: getaddrinfo: %s\n", gai_strerror(rw) );
    return -1;
  }
  ...
  for(p = host_info; p != NULL; p = p->ai_next) 
  {
    ...
    if( address == NULL )
    {
      ...
      {
	perror("open_connection: setsockopt: error setting socket options");
	return -1;
      }
    ...
    else if( type == SOCK_STREAM )
    {
      ...
      {
	perror( "open_connection: open_connection: error on connecting" );
	continue;
      }
    }
...
void close_connections()
{
  ...
  {
    close( connection_list[ connection_id ].sockfd );
    //Undefined behavior
